...
8. **Supabase Auth + Sync (§16/§17) — פרויקט אמיתי כבר קיים ומוגדר
   (ראה §17), עדיין לא נבדק סנכרון בפועל.** `.env` מקומי כבר מלא
   (הערכים ב-§17 למי שצריך למלא מחדש בסשן headless). מה שנשאר:
   `npx expo start`, הזנת אימייל אמיתי, קוד OTP, "Sync now", ואז
   בדיקה בין 2 מכשירים/התקנות. §16 להוראות המלאות, §17 למה שכבר בוצע.
