...
| 9 | **הוסרה מחוות ה-edge-swipe-back של iOS** (`gestureEnabled: false` ב-`_layout.tsx`) | דווח (במכשיר אמיתי) רקע לבן מהבהב מתחת למסכים באמצע המחווה, למרות `SystemUI.setBackgroundColorAsync`, `contentStyle` ו-`userInterfaceStyle: dark` שכבר קיימים (ראו קומיט `d986672`). באג ידוע ב-native-stack/react-native-screens בתרחיש הזה — אין תיקון JS-side אמין, ובפרט `userInterfaceStyle` ממילא לא נטען תחת Expo Go. תואם להחלטה המקבילה ב-Android (`predictiveBackGestureEnabled: false`). כפתור החזרה עדיין עובד עם אותה אנימציה. אם עולה dev build אמיתי בעתיד ורוצים לנסות שוב — לבדוק קודם גרסה חדשה יותר של `react-native-screens`. |
...
1. ~~Timer notification אמיתי (push/background) בסוף סשן~~ — **הושלם
   בסשן הזה, ראה §12.**
2. ~~"זמן להשלמה" (HLTB)~~ — **נבדק מול האתר החי, נמצא שבור (הגנת
   אנטי-בוט חדשה של HLTB, לא רק endpoint שהשתנה) — הוחלט לוותר על
   סנכרון חי בינתיים, ראה §18 לפרטים המלאים. הקוד נשאר כמו שהוא
   (נופל בחזרה ל"אין נתון" בשקט, לא קורס).**
...
## 15. "זמן להשלמה" (HLTB) — קוד קיים שבור מול האתר החי, ראה §18
...
## 18. HLTB — נבדק מול האתר החי, ונמצא ש-HLTB הוסיפו הגנת אנטי-בוט

המשך ישיר ל-§15. הסוכן הזה בדק סוף-סוף מול `howlongtobeat.com` חי
(דרך המשתמש, לא מהסביבה המרוחקת — היא עדיין חסומה לאותו domain,
ראה §15/§17 המקוריים). **המסקנה: הקוד הקיים לא יעבוד יותר, ולא בגלל
regex פשוט שהתיישן כמו שהונח.**

- **מה שהשתנה לטובה:** נתיב ה-API כבר לא דינמי. הוא קבוע:
  `POST https://howlongtobeat.com/api/search/site`. אין יותר צורך
  לחלץ אותו מה-JS bundle (`_next/static/chunks/pages/_app-*.js` —
  ה-pattern הזה כבר לא קיים בדף הבית בכלל, מכאן השגיאה
  `Could not locate HLTB app bundle script` שהתקבלה קודם).
- **מה שהשתנה לרעה — הגנת אנטי-בוט אמיתית:** הבקשה חייבת לכלול 3
  headers/שדות שנוצרים דינמית ע"י ה-JS האמיתי של האתר:
  - `x-hp-key` (למשל `ign_2ce62c80`) ו-`x-hp-val` (למשל
    `3e27c6b7a87546c3`) — גם מופיעים כשדה בתוך ה-JSON payload עצמו
    (`{ign_2ce62c80: "3e27c6b7a87546c3", ...}`), בשם השדה הדינמי הזה.
  - `x-auth-token` — Base64 שמפוענח ל:
    `{timestamp_ms}::{IP של הלקוח}|{User-Agent}|{hp-key}|{hp-val}.{hash בגודל SHA-256, כנראה HMAC}`.
    ה-hash הזה כנראה מחושב עם סוד/מפתח שקיים רק בתוך ה-JS המעורבל
    שלהם (obfuscated) — **לא נמצאה דרך לחשב אותו מ-Node.js פשוט בלי
    להריץ את ה-JS האמיתי שלהם** (למשל בדפדפן headless).
- **גם פורמט הזמנים בתשובה השתנה:** השדות `comp_main`/`comp_plus`/
  `comp_100`/`comp_all` הם עכשיו **שניות**, לא שעות עשרוניות כמו
  שהקוד הישן (`hoursToMinutes` ב-`hltb.ts`) הניח. (אומת מול Hades:
  `comp_main: 84894` שניות ≈ 23.5 שעות — סביר. הקוד הישן היה מכפיל
  ב-60 כאילו זו כמות שעות, ומקבל ~85,000 שעות — שגיאה קשה, לא רק
  שינוי קטן בפורמט.)

**החלטה (בהתייעצות עם המשתמש):** **לוותר על סנכרון חי ל-HLTB בינתיים.**
לא ננסה headless browser (Playwright) כדי לרוץ מול ה-JS האמיתי שלהם —
זה עלות/מורכבות גדולה (תלות כבדה בפרוקסי dev, זמן תגובה איטי בהרבה,
עוד נקודת כשל) עבור תכונה נלווית בלבד שכבר לא נכנסת ל-sessionFit/
מנוע ההמלצה (§15 המקורי). **בדיוק ההחלטה שה-SPEC כבר קיבל מראש
(§7): HLTB אין לו API רשמי, לא אמינים עליו בליבה — וזה התממש בפועל,
בצורה חמורה יותר משצפוי (הגנת אנטי-בוט, לא רק endpoint שהשתנה).**

**מה שזה אומר בפועל, ולמה לא לגעת בקוד:**
- `tools/igdb-proxy/hltb.ts`/`hltbRepo.ts` **נשארים כמו שהם**. הם
  כבר בולעים כל שגיאה בשקט (`enrichGameWithHltb` best-effort, לא
  await מה-caller) — כלומר הפיצ'ר כבר היום נופל בחזרה ל"אין נתון"
  בצורה תקינה, בלי קריסה, בדיוק כמו שתוכנן ל-מקרה כשל. אין דחיפות
  לתקן קוד כי שום דבר לא שבור מבחינת יציבות האפליקציה — רק שהנתון
  "Time to beat" לא יופיע לעולם למשחקים חדשים (הישנים ששמורים ב-DB
  מסשן קודם, אם היו, נשארים).
- **אם מישהו ירצה לנסות שוב בעתיד** (לא בתוכנית עכשיו): הדרך היחידה
  הריאלית היא headless browser אמיתי (Playwright/Chromium, שכבר קיים
  ב-tooling הזה) שינווט בפועל ל-howlongtobeat.com ויבצע את החיפוש
  כמו משתמש אמיתי — לא ניסיון נוסף לחשב את ה-hash ידנית. זה ל-`IDEAS.md`
  אם ירצו, לא נבנה עכשיו.
- אם עדיין רוצים "זמן להשלמה" כפיצ'ר בעתיד: אפשרות אחרת שלא נבדקה —
  API בתשלום/רשמי חלופי (אם קיים משהו כזה כיום), או קלט ידני של
  המשתמש. גם זה ל-IDEAS.md.
...
## 17. פרויקט Supabase אמיתי נוצר — עדיין לא אומת שסנכרון עובד קצה-לקצה

המשתמש יצר פרויקט Supabase אמיתי (org `wjlqhibattqabivhrblo`, project ref
`hyxkdoeaidfmocmldcib`) והריץ בהצלחה את `supabase/migrations/0001_sync_tables.sql`
ב-SQL Editor. שלבים 1-3 מ-§16 הושלמו:

1. ✅ פרויקט נוצר.
2. ⚠️ **Email provider ב-Authentication → Sign In / Providers — לא אומת
   במפורש שהוא מופעל.** ברירת המחדל של Supabase היא מופעל, וזה לא נבדק
   שלילית (לא דווח על בעיה), אבל גם לא אושר בפירוש מול המשתמש. לבדוק
   לפני שסומכים על שליחת קוד OTP.
3. ✅ המיגרציה רצה בהצלחה (אחרי timeout ראשוני שנבע מפרויקט חדש שעוד
   לא סיים provisioning — לא באג, רק סבלנות).
4. ✅ Credentials התקבלו והוכנסו ל-`.env` **מקומי בלבד** (קובץ
   ב-`.gitignore`, לא נכנס ל-git אף פעם):
   - `EXPO_PUBLIC_SUPABASE_URL=https://hyxkdoeaidfmocmldcib.supabase.co`
   - `EXPO_PUBLIC_SUPABASE_ANON_KEY=eyJhbGci...rPQDKE` (JWT anon key —
     המשתמש שלח גם מפתח בפורמט החדש `sb_publishable_...`, אבל ה-anon
     JWT הוא מה שנשמר כי זה מה שהקוד הקיים ב-`src/lib/supabase/client.ts`
     מצפה לו עם `@supabase/supabase-js`). ה-URL אומת משתי מקורות
     עצמאיים (project ref בלינקים של ה-Dashboard + `"ref"` claim בתוך
     ה-JWT עצמו) — לא רק הנחה.

   **חשוב לסוכן הבא:** מכיוון ש-`.env` לא נכנס ל-git, סשן חדש/headless
   **לא** יראה את הקובץ הזה — צריך למלא אותו מחדש. מאחר וה-anon key
   מיועד להיות ציבורי (מוגן ע"י RLS, בדיוק כמו שמתועד ב-§16 למעלה),
   הערכים נשמרים כאן כטקסט רגיל כדי לא להצטרך לשאול את המשתמש שוב —
   פשוט תעתיק אותם ל-`.env` לפי `.env.example`.

5. ❌ **לא בוצע/לא ניתן לבצע מהסביבה הזו:** הרצת האפליקציה בפועל
   (`npx expo start`), הזנת אימייל אמיתי, קבלת קוד OTP, ובדיקת "Sync
   now" בין שני מכשירים/התקנות (§16 סעיפים 5-6). **גם בדיקת קישוריות
   בסיסית (`curl` ל-`*.supabase.co`) נכשלה מהסביבה המרוחקת הזו —
   `connect_rejected` ברמת ה-proxy הארגוני** (אותה מגבלת רשת שכבר
   תועדה לגבי HLTB ב-§15 — לא ספציפי ל-Supabase). **המשמעות: הקוד
   וה-`.env` מוכנים, אבל שום קריאת רשת אמיתית ל-Supabase עוד לא בוצעה
   מכל סביבת פיתוח (לא headless, לא Windows המשתמש) — הצעד הבא חייב
   להיות בפועל על הטלפון/המחשב של המשתמש.**

**נבדק בסשן הזה:** `npm install` + typecheck (מלבד `global.css` הידוע)
+ lint + 118 טסטים — כולם עברו. שום שינוי קוד לא בוצע (§16 כבר היה
מוכן בקוד מסשן קודם) — הסשן הזה הוא כולו הגדרת התשתית האמיתית מול
Supabase.

**הצעד הבא בפועל, לפי §16 סעיפים 5-6:** `npx expo start`, הגדרות →
הזנת אימייל אמיתי → אימות שקוד ה-OTP מגיע (כולל בדיקת ספאם) → הזנת
קוד → "Sync now" לא זורק שגיאה → ואז הבדיקה האמיתית: הוספת משחק/סשן
...
### אומת בפועל על מכשיר אמיתי — Auth+Sync עובד קצה-לקצה ✅

אחרי תיקון הבאג למעלה, המשתמש הריץ בפועל על הטלפון שלו והשלים את כל
זרימת §16 סעיפים 5-6:

1. `npx expo start -c` עלה בלי קריסה (התיקון ל-migration 0002 עבד).
2. **מלכודת חדשה שנתגלתה: קוד ה-OTP לא הגיע בכלל — הגיע קישור
   ("Confirm your email" / "Sign in link") שמפנה ל-`localhost:3000`
   ונכשל.** סיבת שורש: `signInWithOtp` שולח בפועל את תבנית
   **"Confirm signup"** (למשתמש חדש) ואת **"Magic Link"** (למשתמש
   קיים) מה-Dashboard של Supabase — וברירת המחדל של שתיהן מכילה רק
   `{{ .ConfirmationURL }}`, לא `{{ .Token }}` (הקוד בן 6 הספרות
   שהאפליקציה מצפה לו ב-`verifyOtp`). **זה בדיוק החשש שכתוב ב-§16
   סעיף 2 המקורי — התממש בפועל.**
3. **תיקון (בדשבורד, לא בקוד):** Authentication → Emails → Templates.
   כדי לערוך את ה-HTML הגולמי של תבנית (ולהוסיף `{{ .Token }}`),
   Supabase **דורש SMTP מותאם אישית** — שרת המייל המשותף שלהם לא
   מאפשר עריכת source. הוגדר Gmail SMTP (host `smtp.gmail.com`,
   port `587`, App Password מ-Google — דורש 2FA מופעל בחשבון
   Google). **אחרי** שה-SMTP נשמר ומופעל, נערכו **שתי** התבניות
   (Confirm signup **וגם** Magic Link — כל אחת נשלחת בנסיבות אחרות)
   כדי להוסיף שורת `{{ .Token }}`.
4. אחרי זה: קוד בן 6 ספרות התחיל להגיע במייל, `verifyOtp` הצליח,
   "Sync now" רץ בלי שגיאה, ו**אומת ישירות ב-Table Editor של
   Supabase** שהשורות (profile, `user_games`) הגיעו בפועל לפרויקט
   המרוחק.

**מסקנה: §16 (Supabase Auth+Sync) אומת קצה-לקצה על מכשיר אמיתי ועובד.**
מה שנשאר עדיין לא אומת (לא קריטי, פחות סיכון): הבדיקה הדו-מכשירית
המלאה (התקנה שנייה עם אותו אימייל, ולוודא שספרייה/היסטוריה מסתנכרנות
חזרה) — §16 סעיף 6. אם מישהו ירצה לבדוק את זה, הדרך הקלה ביותר היא
למחוק את האפליקציה מהטלפון ולהתקין מחדש (DB מקומי ריק מאפס), להתחבר
עם אותו אימייל, ולבדוק "Sync now" מוריד את הנתונים חזרה.

**לסוכן הבא: אם אי-פעם צריך פרויקט Supabase נוסף (לדוגמה staging),
זכור מראש להגדיר Custom SMTP + לערוך את שתי התבניות (Confirm signup
ו-Magic Link) עם `{{ .Token }}` **לפני** הבדיקה הראשונה — זה יחסוך
בדיוק את סבב הדיבוג הזה.**
...
7. ~~"זמן להשלמה" מול HLTB חי~~ — **נבדק, נמצא שבור בגלל הגנת
   אנטי-בוט חדשה של HLTB (לא endpoint פשוט שהשתנה). הוחלט לוותר על
   סנכרון חי בינתיים — ראה §18. אין יותר צעד פעולה כאן.**
8. ~~Supabase Auth + Sync~~ — **הושלם ואומת בפועל, ראה §17 (כולל
   מלכודת SMTP/Email Templates שנתגלתה ותוקנה).** נשאר פתוח רק חלק
